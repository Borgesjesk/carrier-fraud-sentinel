...
/**
 * TransactionTest verifies Transaction domain model behavior.
 *
 * TESTING STRATEGY:
 * - Constructor validation: Fail-Fast principle
 * - Computed properties: Correctness of calculations
 * - Happy path: Valid transaction creation
 * - Unhappy paths: All possible validation failures
 *
 * NAMING CONVENTION:
 * testX_YShouldZ
 * - X = what's being tested (ConstructorValidation, PaymentSuccessRate, etc.)
 * - Y = the scenario (NullCarrierName, ZeroPrice, etc.)
 * - Z = expected result (ThrowIllegalArgumentException, ReturnCorrectValue, etc.)
 *
 * @DisplayName = human-readable test name (shows in IDE)
 */
...
class TransactionTest {
    ...
    void testConstructor_ValidData_CreatesTransaction() {
        // ARRANGE
    ...
    void testConstructor_NullCarrierName_ThrowsException() {
        assertThrows(
